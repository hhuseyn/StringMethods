...
namespace StringMethodsTask
{
    internal class Program
    {
        ...
        public static void FixMailAdress(string[] mailAdress)
        {
            string[] userData = { };
            string infoMail;
            string result;
            foreach (string mail in mailAdress)
            {
                ...
                //iceride olan reqemleri silir,  '-' ve '_' bu simvollari '.' simvolu ile evez edir.
                userData[0] = (Regex.Replace(userData[0], "(\\d)", string.Empty)).Replace('-', '.').Replace('_', '.');
                infoMail = userData[1];

                if (userData[0].Contains('.'))
                {
                    userData = userData[0].Split('.');
                    result = $"\nFirstName: {userData[0]}\nLastName : {userData[1]} \nMail     : {userData[0]}.{userData[1]}@{infoMail}\nDomain   : {(infoMail.Split('.'))[0]}\nExtention: {(infoMail.Split('.'))[1]}";
                }
                else
                {
                    result = "Bu mail duzgun deyil";
                }
